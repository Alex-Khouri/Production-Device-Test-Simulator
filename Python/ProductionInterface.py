...
class RunTestWorker(QObject):
	...
	def saveGraph(self):
		...
		if self.outputFormat == PDF:
			...
			self.printOut("Invalid output format select")
	...
	def updateGraph(self):
		...
		displayTimestamps = self.timestamps[minVal:]
		displaymvData = self.mvData[minVal:]
		displaymaData = self.maData[minVal:]
		self.canvas.axes.plot(displayTimestamps, displaymvData, 'r', label="Voltage")
		self.canvas.axes.plot(displayTimestamps, displaymaData, 'b', label="Current")
		self.canvas.axes.set_xlabel("Time (milliseconds)")
	...
	def sendMessage(self, message):
		...
		# self.printOut(f"Message Sent: {message}")
	...
	def receiveMessages(self):
		...
		if self.running:
			...
			# self.printOut(f"Message Received: {message}")
...
class AppGUI(QtWidgets.QMainWindow):
	def __init__(self, *args, **kwargs):
		...
		self.widget.setWindowIcon(QIcon("Images/rocket_lab_logo.png"))
		...
		self.labelDisplayCount = QLabel(text="Live Displayed Values:")
		...
		self.linePortDevice.setValidator(QIntValidator(1, 65545))
		self.linePortInterface = QLineEdit()
		self.linePortInterface.setValidator(QIntValidator(1, 65545))
		self.lineDuration = QLineEdit()
		self.lineDuration.setValidator(QIntValidator(bottom=1))
		self.lineInterval = QLineEdit()
		self.lineInterval.setValidator(QIntValidator(bottom=1))
		...
		self.labelDisplayCountMin = QLabel(text="10")
		self.labelDisplayCountMax = QLabel(text="100")
		...
		self.layout.addWidget(self.checkGenerateFile,		2, 0, 1, 1)
		...
		self.layout.addWidget(self.buttonOutputLocation,	8, 0, 1, 1)
		self.layout.addWidget(self.lineOutputLocation,		8, 1, 1, 4)
		self.layout.addWidget(self.labelInfo, 	 			9, 0, 1, 2)
		self.layout.addWidget(self.textOutput,		 		10, 0, 1, 5)
		self.layout.addWidget(self.canvas,					0, 5, 11, 1)
	...
	def manualFieldValidation(self):	# Used to provide additional validation of integer fields
		return int(self.linePortDevice.text()) >= 1 and int(self.linePortDevice.text()) <= 65535 and \
			int(self.linePortInterface.text()) >= 1 and int(self.linePortInterface.text()) <= 65535 and \
			int(self.lineDuration.text()) >= 1 and int(self.lineInterval.text()) >= 1
...
try:
	...
	gui.printOut("Error with program execution!")
finally:
	exit(app.exec())